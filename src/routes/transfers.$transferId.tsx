  ...
  if (w.status === "envoye" || w.current_step >= 3) {
    return (
      <Badge className="bg-success/15 text-success border-0 gap-1">
        <CheckCircle2 className="h-3 w-3" /> Virement validé
      </Badge>
    );
  }
  if (w.status === "rejete") {
    return (
      <Badge className="bg-destructive/15 text-destructive border-0 gap-1">
        <XCircle className="h-3 w-3" /> Virement rejeté
      </Badge>
    );
  }
  const targets = [63, 88, 100];
  const t = targets[w.current_step] ?? 100;
  if (w.progress >= t && w.current_step < 3) {
    return (
      <Badge className="bg-warning/15 text-warning border-0 gap-1">
        <Lock className="h-3 w-3" /> Vérification de conformité
      </Badge>
    );
  }
  return (
    <Badge className="bg-info/15 text-info border-0 gap-1">
      <Loader2 className="h-3 w-3 animate-spin" /> Traitement bancaire
    </Badge>
  );
}
  ...
  return (
    <div className="container mx-auto max-w-3xl space-y-6 px-4 pb-28 pt-8 lg:pb-10">
      <Button variant="ghost" size="sm" onClick={() => navigate({ to: "/transfers" })}>
        <ArrowLeft className="mr-2 h-4 w-4" /> Tous les virements
      </Button>
        ...
        <CardContent className="p-6 md:p-8 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-white/70">
              <ShieldCheck className="h-3.5 w-3.5" /> HSBC BANK · Ordre de virement
            </div>
            {statusBadge(w)}
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-3">
            <Field label="Bénéficiaire" value={w.beneficiary} />
            <Field label="Banque" value={w.bank_name} />
            <Field label="Référence" value={w.reference ?? "—"} />
            <Field label="IBAN" value={w.iban} mono />
            {w.bic && <Field label="BIC" value={w.bic} mono />}
            <Field label="Émis le" value={formatDateTime(w.created_at)} />
          </div>
        </CardContent>
      </Card>

      {/* Processus / blocage / code */}
      <Card>
        <CardContent className="p-6 space-y-4">
          <h2 className="font-serif text-xl">Suivi du virement</h2>
        ...
        <CardContent className="p-6">
          ...
          <ol className="relative space-y-4 border-l border-border pl-5">
            <TimelineItem
              ...
              title="Ordre de virement émis"
              desc={`Vers ${w.beneficiary} (${formatCurrency(Number(w.amount))})`}
              done
            />
            {w.current_step >= 1 && (
              <TimelineItem
                when={formatDateTime(w.step_started_at)}
                title="Étape de conformité validée"
                desc="Code de déblocage saisi avec succès."
                done
              />
            )}
            {w.current_step >= 2 && (
              <TimelineItem
                when={formatDateTime(w.step_started_at)}
                title="Vérification renforcée validée"
                desc="Le virement poursuit son acheminement vers le bénéficiaire."
                done
              />
            )}
            {isFinal && (
              <TimelineItem
                when={w.processed_at ? formatDateTime(w.processed_at) : "À l'instant"}
                title="Virement exécuté"
                desc="Les fonds ont été crédités."
                done
              />
            )}
          </ol>
        ...
        <Card>
          <CardContent className="p-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">Justificatif bancaire disponible</p>
                <p className="text-xs text-muted-foreground">
                  Téléchargez le récapitulatif officiel de cet ordre de virement.
                </p>
              </div>
            </div>
            <Button onClick={() => window.print()} className="shadow-glow">
              <Download className="mr-2 h-4 w-4" /> Télécharger / Imprimer
            </Button>
          </CardContent>
        </Card>
      )}
